...
// echo $password.'<br>'.PwdHash($input_password,substr('',0,9));exit();
		...
		//echo "test";exit();
			...
			$_SESSION['HTTP_USER_AGENT'] = md5($_SERVER['HTTP_USER_AGENT']);
				...
				//echo "SELECT od_id,od_session FROM js_shop_order_item WHERE od_session = '$sid' and `user_id` = '$userID' and deleted = '0'";exit();
				 $get_current_session_ID_shop_item = sqlQUERY_LABEL("SELECT od_id,od_session FROM js_shop_order_item WHERE od_session = '$sid' and `user_id` = '$userID' and deleted = '0'") or die(sqlERROR_LABEL());
				...
				if(sqlNUMOFROW_LABEL($get_current_session_ID_shop_item)){
					...
					sqlQUERY_LABEL("UPDATE js_shop_order_item SET `user_id` = '$userID',`od_id` = '$od_id', `createdby` = '$userID' WHERE user_id='0' and od_session = '$sid'");
				}
		...
		//$data = array('status' => 'Error', 'msg' => 'Password is Correct');
			...
			$_SESSION['HTTP_USER_AGENT'] = md5($_SERVER['HTTP_USER_AGENT']);
				...
				//echo "SELECT od_id,od_session FROM js_shop_order_item WHERE od_session = '$sid' and `user_id` = '$userID' and deleted = '0'";exit();
				 $get_current_session_ID_shop_item = sqlQUERY_LABEL("SELECT od_id,od_session FROM js_shop_order_item WHERE od_session = '$sid' and deleted = '0'") or die(sqlERROR_LABEL());
