...
if ($action == "delete" && $id != '') {
	...
	$sqlWhere= "categoryID=$id";

	if(sqlACTIONS("UPDATE","js_category",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'category.php?code=5'; </script>";
	}
}
    ...
    <script>
      $(function(){
        ...
        $('#categoryLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONcategory.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "categoryname" },
            { "data": "status" },
		  ...
		  "columnDefs": [{
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="category.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__category.php?type=delete&delete_id='+SELECTED_ID+'',function(){
	...
	function  togglestatusITEM(category_id, status) { 
		...
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__category.php?type=changestatus&categoryID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
	...
	// function get_encoded_categoryname(){
		
	// var search_value =document.getElementById("categoryname").value;
	// b64encode = btoa(unescape(encodeURIComponent(search_value)))

	// document.getElementById('encrypt_category_name').value = b64encode;
	...
	// $('#encrypt_category_name').parsley();
		// window.ParsleyValidator.addValidator('checkcategoryname', {
		  // validateString: function(value)
		  // {
			// return $.ajax({
			  // url:'engine/ajax/ajax_category_name.php',
			  // method:"POST",
   ...
   $(document).ready(function(){
    $('#categoryname').parsley();
    var old_category_name = document.getElementById( "encrypt_category_name" ).value;
    window.ParsleyValidator.addValidator('checkcategoryname', {		 
      validateString: function(value)
      {
        return $.ajax({
          url:'engine/ajax/ajax_category_name.php',
          method:"POST",
          data:{category_name:value ,old_category_name:old_category_name},
          dataType:"json",
          success:function(data)
          {
   ...
   $(document).ready(function(){
    $('#categorycode').parsley();
    var old_category_name = document.getElementById( "encrypt_categorycode" ).value;
    window.ParsleyValidator.addValidator('checkcategorycode', {		 
      validateString: function(value)
      {
        return $.ajax({
          url:'engine/ajax/ajax_category_code.php',
          method:"POST",
          data:{category_code:value ,old_category_code:old_category_name},
          dataType:"json",
          success:function(data)
          {
