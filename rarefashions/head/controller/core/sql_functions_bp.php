...
function get_Minisales($selected_id,$type){ 
	if($type== 'bill_refno') {
		$soldout_bill = sqlQUERY_LABEL("SELECT bill.billrefno,bill.cusID FROM `js_billitem` as item , js_bill as bill where item.categoryID = '$selected_id' and item.billID = bill.billID and bill.deleted = 0 and item.deleted = 0 and bill.status != '0'") or die("Unable to get records:".mysql_error());
	...
	if($type== 'billitemprdt') {
		$soldout_bill = sqlQUERY_LABEL("SELECT item.billitemprdt,item.billitemtype FROM `js_billitem` as item , js_bill as bill where item.categoryID = '$selected_id' and item.billID = bill.billID and bill.deleted = 0 and item.deleted = 0 and bill.status != '0' group by item.billitemprdt") or die("Unable to get records:".mysql_error());
	...
	if($type== 'billitemqty') {
		$soldout_bill = sqlQUERY_LABEL("SELECT sum(item.billitemqty) as total FROM `js_billitem` as item , js_bill as bill where item.categoryID = '$selected_id' and item.billID = bill.billID and bill.deleted = 0 and item.deleted = 0 and bill.status != '0' group by item.billitemprdt") or die("Unable to get records:".mysql_error());
		$count_today_sales = mysql_num_rows($soldout_bill);
		if($count_today_sales > 0){
			while($collect_sold_today = mysql_fetch_array($soldout_bill)) {
