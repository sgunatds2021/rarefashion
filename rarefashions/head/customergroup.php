...
if ($action == "delete" && $id != '') {
	...
	$sqlWhere= "customergroupID=$id";

	if(sqlACTIONS("UPDATE","js_customergroup",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'customergroup.php?code=1'; </script>";
	}
}
  ...
  <body>
    ...
    <script>
      $(function(){
        ...
        $('#cusgroupLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONcustomergroup.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "categoryname" },
            { "data": "status" },
		  ...
		  "columnDefs": [{
				...
				"render": function ( data, type, row ) {
				  switch(data) {
					   case '1' : return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="customergroupstatus-'+row.modify+'" id="customergroupstatus'+row.modify+'" checked="" onChange="togglestatusITEM('+row.modify+', '+data+');"> <label class="custom-control-label" for="customergroupstatus'+row.modify+'">Yes</label></div>'; break;
					   case '0' : return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="customergroupstatus--'+row.modify+'" id="customergroupstatus'+row.modify+'" onChange="togglestatusITEM('+row.modify+', '+data+');"> <label class="custom-control-label" for="customergroupstatus'+row.modify+'">Yes</label></div>'; break;
					}
				}
			 }, {
			  	"targets": 3,
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="customergroup.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
<?php } ?>
	...
	alert(deleting_id);
		...
		$('.receiving-delete-data').load('view/x__customergroup.php?type=delete&delete_id='+SELECTED_ID+'',function(){
			$('#pleasewait-loader').hide();
			$('#deleteDATA').modal({show:true});
		});
	}

	function  togglestatusITEM(customergroup_id, status) { 
	
		var SELECTED_ID = customergroup_id;
		var SELECTED_STATUS = status;
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__customergroup.php?type=changestatus&customergroupID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
  ...
  <?php
    ...
    if($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add Customer Group.', 'error');  
    }
