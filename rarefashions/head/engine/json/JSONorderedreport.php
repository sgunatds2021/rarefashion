...
if($status == '8'){
 	$filterby = "and od_status = '$status'";
} else {
	$filterby = "and od_status = '6'";
}
...
echo '"data":[';
	...
	$list_datas = sqlQUERY_LABEL("SELECT * FROM `js_shop_order` where deleted = '0' {$filterby} {$filterby_date} order by od_id desc") or die("Unable to get records:".sqlERROR_LABEL());
