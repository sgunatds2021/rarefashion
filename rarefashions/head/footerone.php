...
if ($action == "delete" && $id != '') {
	...
	$sqlWhere= "footeroneID=$id";

	if(sqlACTIONS("UPDATE","js_footerone",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'footerone.php?code=1'; </script>";
	}
}
    ...
    <script>
      $(function(){
        ...
        $('#categoryLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONfooterone.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "footeronetitle" },
            { "data": "status" },
		  ...
		  "columnDefs": [{
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="footerone.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
//////////////////////////////////      
      if($route == 'add' || $route == 'edit') {  ?>
       	...
       	if($("#footeronedescription").length > 0){
			tinymce.init({
				selector: "textarea#footeronedescription",
				theme: "modern",
				menubar: false,
				min_height:300,
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__footerone.php?type=delete&delete_id='+SELECTED_ID+'',function(){
			$('#pleasewait-loader').hide();
			$('#deleteDATA').modal({show:true});
		});
	}

	function  togglestatusITEM(footerone_id, status) { 
		var SELECTED_ID = footerone_id;
		var SELECTED_STATUS = status;
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__footerone.php?type=changestatus&footeroneID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
