...
if ($action == "delete" && $id != '') {
	...
	$sqlWhere= "homesliderID=$id";

	if(sqlACTIONS("UPDATE","js_homeslider",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'homeslider.php?code=1'; </script>";
	}
}
    ...
    <script>
      $(function(){
        ...
        $('#categoryLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONhomeslider.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "homeslidertitle1" },
            { "data": "homeslidertitle2" },
            { "data": "status" },
            { "data": "modify" }
          ],
		  "columnDefs": [{
...
//////////////////////////////////      
      if($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#homesliderimage").change(function() {
            uploadImage(this);         
        });
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__homeslider.php?type=delete&delete_id='+SELECTED_ID+'',function(){
			$('#pleasewait-loader').hide();
			$('#deleteDATA').modal({show:true});
		});
	}

	function  togglestatusITEM(homeslider_id, status) { 
		var SELECTED_ID = homeslider_id;
		var SELECTED_STATUS = status;
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__homeslider.php?type=changestatus&homesliderID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
