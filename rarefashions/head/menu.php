...
if ($action == "delete" && $id != '') {
	...
	if(sqlACTIONS("UPDATE","js_menu",$arrFields,$arrValues, $sqlWhere)) {
		...
		echo "<script type='text/javascript'>window.location = 'menu.php?code=1'; </script>";
	}
}
    ...
    <script>
      $(function(){
        ...
        $('#categoryLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONmenu.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "menu_name" },
            { "data": "status" },
		  ...
		  },
		  {
			  "targets": 1,
				...
				"render": function ( data, type, row, full, meta ) {
					if(row.menu_type == '2'){
						return row.menu_name+ '&nbsp;<sup><small><span class="badge badge-danger"> Mega Menu </span></small></sup>';
					} else {
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="menu.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__menu.php?type=delete&delete_id='+SELECTED_ID+'',function(){
	...
	function  togglestatusITEM(menu_ID, status) { 
		...
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__menu.php?type=changestatus&menu_ID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
...
function showmenublock(menu_type){
