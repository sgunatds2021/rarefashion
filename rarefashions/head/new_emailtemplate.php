...
if ($action == "delete" && $id != '') {
	...
	if(sqlACTIONS("UPDATE","js_new_emailtemplate",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'new_emailtemplate.php?code=1'; </script>";
	}
}
    ...
    <script>
      ...
      $(function(){
        'use strict'
		 ...
		 <?php if($route == '') {  ?>
			 $('#new_emailtemplate').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONnew_emailtemplate.php',
           "columns": [
            { "data": "count" },
            { "data": "template_name" },
            { "data": "email_subject" },
		  ...
		  "columnDefs": [{
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="new_emailtemplate.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
//////////////////////////////////      
  ...
  <?php } ?>
		...
		$('.receiving-delete-data').load('view/x__new_emailtemplate.php?type=delete&delete_id='+SELECTED_ID+'',function(){
	...
	function  togglestatusITEM(emailtemplate_id, status) { 
		...
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__new_emailtemplate.php?type=changestatus&template_ID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
