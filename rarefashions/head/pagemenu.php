...
//include_once('check_restricted.php');
//update query
if ($action == "delete" && $id != '' && $restricted_access !="0") {
	//Insert query
	...
	if(sqlACTIONS("UPDATE","js_pagemenu",$arrFields,$arrValues, $sqlWhere)) {
		...
		echo "<script type='text/javascript'>window.location = 'pagemenu.php?code=1'; </script>";
	}
}
  ...
  <body>
    ...
    <script>
      $(function(){
        ...
        $('#PagemenuLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONpagemenu.php',
           "columns": [
			{data: "count"}, //0
			{data: "menu_title"}, //1
			{data: "page_type"}, //2
			{data: "page_name"}, //3
			{data: "sidebar_display"}, //4
			{data: "Parent"}, //5
			{data: "status"}, //6
			{data: "modify"},
          ],
		  ...
		  },
		  {
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="pagemenu.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__pagemenu.php?type=delete&delete_id='+SELECTED_ID+'',function(){
	...
	function  togglestatusITEM(page_ID, status) { 
		...
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__pagemenu.php?type=changestatus&page_ID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
		...
		function showparent() { 
	  var visitor_type = document.getElementsByName('parent');
	  var selectedValue = '';
	  for(var i = 0; i < visitor_type.length; i++){
			if(visitor_type[i].checked){
				selectedValue = visitor_type[i].value;
				if(selectedValue == '0'){
				$('#show_parent').show();
				}else{
				$('#show_parent').hide();
