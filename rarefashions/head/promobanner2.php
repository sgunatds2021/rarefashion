...
if ($action == "delete" && $id != '') {
	...
	if(sqlACTIONS("UPDATE","js_promobanner",$arrFields,$arrValues, $sqlWhere)) {
		...
		echo "<script type='text/javascript'>window.location = 'promobanner2.php?code=1'; </script>";
	}
}

//select
$list_datas = sqlQUERY_LABEL("SELECT * FROM `js_promobanner` where promobannerTYPE ='2' and deleted = '0'") or die("#1-Unable to get records:".mysql_error());
    ...
    <script>
      $(function(){
        ...
        $('#promobanner1_LIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONpromobanner2.php',
           "columns": [
            { "data": "count" },
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__promobanner2.php?type=delete&delete_id='+SELECTED_ID+'',function(){
	...
	function  togglestatusITEM(promobanner1_id, status) { 
		...
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__promobanner2.php?type=changestatus&promobannerID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
	...
	function  promoBANNERIMAGE(get_staff_id) { 
		...
		$('.receiving-delete-data').load('engine/ajax/ajax_promobanner1.php?type=promobanner_img_two&promobannertwo_id='+SELECTED_ID+'',function(){
