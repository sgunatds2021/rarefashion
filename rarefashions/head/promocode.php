...
if ($action == "delete" && $id != '') {
	...
	$sqlWhere= "promocode_id=$id";

	if(sqlACTIONS("UPDATE","js_promocode",$arrFields,$arrValues, $sqlWhere)) {
		echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
		echo "<script type='text/javascript'>window.location = 'promocode.php?code=3'; </script>";
	}
}
	...
	//Expiry Date Datepicker
      ...
      $(function(){
        ...
        $('#promocodeList').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSON_promocode.php',
           "columns": [
            { "data": "counter" },
            { "data": "promocode_name" },
            { "data": "promocode_type" },
            ...
            { "data": "promocode_expiry_date" },
            { "data": "promocode_option" },
            { "data": "status" },
            { "data": "modify" }
          ],
		  "columnDefs": [{
				...
				"render": function ( data, type, full, meta ) {
					return '<a title="Click to edit" href="promocode.php?route=edit&id='+data+'" class="btn btn-light btn-icon"><i class="fa fa-pencil-alt"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM('+data+');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
					//return '';
				}
			 } 
		  ],
...
<?php } ?>
	...
	function  deleteITEM(deleting_id) { 
		...
		$('.receiving-delete-data').load('view/x__promocode.php?type=delete&delete_id='+SELECTED_ID+'',function(){
			$('#pleasewait-loader').hide();
			$('#deleteDATA').modal({show:true});
		});
	}

	function  togglestatusITEM(promocode_id, status) { 
		var SELECTED_ID = promocode_id;
		var SELECTED_STATUS = status;
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.receiving-delete-data').load('view/x__promocode.php?type=changestatus&promocode_id='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
	
	$('#maximum_dis_amount').hide();
	function showmindiscamount()
	{
		$('#maximum_dis_amount').show();
		$('#max_discount_amt').prop('required', true);
	}
	
	function hidemindiscamount()
	{
		$('#maximum_dis_amount').hide();
		$('#max_discount_amt').prop('required', false);
	}
