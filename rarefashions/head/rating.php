...
if ($action == "delete" && $id != '') {
	...
	if(sqlACTIONS("UPDATE","js_tariff",$arrFields,$arrValues, $sqlWhere)) {
		...
		echo "<script type='text/javascript'>window.location = 'tariff.php?code=3'; </script>";
	}
  }
  ...
  <body>
    ...
    <script>
      $(function(){
        ...
        $('#LIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONrating.php?show=<?php echo $show; ?>',
           "columns": [
            { "data": "count" },
            { "data": "product" },
            { "data": "od_ref_no" },
		  ...
		  },
		  {
				...
				"searchable": false,
				 "render": function ( data, type, row, full, meta ) {
					 if(data != 'N/A'){
						return '<a title="Click to Preview" href="<?php echo SITEHOME; ?>product.php?token='+row.product_ID+'-'+row.product_sku+'">'+row.product+'</a>';
					 } else {
						 return data;
					 }
				} 
			...
			// {
				...
				// "render": function ( data, type, row, full, meta ) {
					// if(data != 'N/A'){
						// return '<a title="Click to Preview" href="<?php echo BASEPATH; ?>order?route=preview&id='+row.od_id+'">'+row.od_ref_no+'</a>';
					// } else {
						// return data;
					// }
				// } 
...
<?php } ?>
	...
	function  togglestatusITEM(review_ID, status) { 
		var SELECTED_ID = review_ID;
		var SELECTED_STATUS = status;
		//alert(SELECTED_ID);
		$('#pleasewait-loader').show();
		$('.sr-only').load('view/x__rating.php?type=changestatus&ratingID='+SELECTED_ID+'&oldstatus='+SELECTED_STATUS+'',function(){
			$('#pleasewait-loader').hide();
			location.reload();
		});
	}
  ...
  <?php
    ...
    if($code == '0') {
	 ...
	 if($code == '3') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Record Deleted Successfully', 'error');  
    }
