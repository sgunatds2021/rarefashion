  ...
  <body>
        ...
        <div class="d-sm-flex align-items-center justify-content-between">
          ...
          <div class="mg-t-20 mg-sm-t-0">
            ...
            <?php } if(in_array($route,array('preview'))){ ?>
				<a href="<?php echo BASEPATH; ?>verifyaccount.php"  class="btn btn-xs btn-secondary btn-icon mg-r-2">Back</a>
	...
	format: 'mm/yyyy'
        ...
        $('#customersalesreportLIST').DataTable({
          //responsive: true,
		  	dom: 'Bfrtip',
          'ajax': 'engine/json/JSONverifyaccount.php?filter_customer=<?php echo $filter_customer; ?>&since_from=<?php echo $since_from; ?>&since_to=<?php echo $since_to; ?>',
           "columns": [
            { "data": "count" },
			{ "data": "customer_name" },
			{ "data": "email" },
		  ...
		  "columnDefs": [{
				...
				"render": function ( data, type, row, full, meta ) {
					if(data == 0){
					return '<span>'+row.useremail+'</span>';
					} else if(data == 1){
					return '<span>'+row.useremail+'<span class="text-success ml-3"><i class="fa fa-check-circle"></i></span>' ;
					}
				}
			 },{
			  	"targets": 3,
				...
				"render": function ( data, type, row, full, meta ) {
					if(data == 0){
					return '<span>'+row.user_phone+'</span>';
					} else if(data == 1){
					return '<span>'+row.user_phone+'<span class="text-success ml-3"><i class="fa fa-check-circle"></i></span>' ;
					}
				}
			 }
		  ],
