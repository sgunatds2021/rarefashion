		...
		if($media_size<(1000*1000))
			{
				...
				if(move_uploaded_file($tmp, $media_path.$promobanner1_image)) {
					...
					//Insert query
					$arrFields=array('`promobannerTYPE`','`promobanner_title`','`display_order`','`display_size`','`promobanner_image`','`banner_link`','`status`','`createdby`');
					$arrValues=array('2',"$promobanner_title","$displayorder","$display_size","$promobanner1_image","$banner_link","$promobannerstatus","$logged_user_id");
					...
					if(sqlACTIONS("INSERT","js_promobanner",$arrFields,$arrValues,'')) {
						...
						if( $save == "save"	) {
							?>
							<script type="text/javascript">window.location = 'promobanner2.php?route=add&code=1' </script>
							<?php
