...
//$check_pagetype =  pageTYPE($action,'getid');

//Insert Operation
if($save_continue == "save_continue") {
	...
	if($status == 'on') { $status = '1'; } else { $status = '0'; }
					...
					$arrFields=array('`role_name`', '`page_allowed`','`createdby`','`status`');
					$arrValues=array("$role_name", "$page_allowed", "$logged_user_id","$status");

					if(sqlACTIONS("INSERT","js_rolemenu",$arrFields,$arrValues,'')) {
						$roleid = sqlINSERTID_LABEL();
...
if( $save == "update" && $hidden_role_id != '') {
	...
	if($status == 'on') { $status = '1'; } else { $status = '0'; }
					...
					if(sqlACTIONS("UPDATE","js_rolemenu",$arrFields,$arrValues, $sqlWhere)) {
						...
						?>
						<script type="text/javascript">window.location = 'rolepermission.php?id=<?php echo $hidden_role_id; ?>&route=add&formtype=notification_config' </script>
						<?php
				...
				<form method="post" enctype="multipart/form-data" data-parsley-validate>
				  ...
				  <div id="stickThis" class="form-group row mg-b-0">
				    ...
				    <div class="col-9 col-sm-6 text-right">
				      <button type="submit" name="save_continue" value="save_continue" class="btn btn-success"><?php echo $__save_continue ?></</button>
				    </div>
				  </div>
				...
				<div class="row">
						<div class="col-lg-4 col-md-4">
							<div class="form-group">
							  ...
							  <input type="text" id="role_name" name="role_name" class="form-control" data-parsley-whitespace="trim" data-parsley-pattern="^[a-zA-Z ]+$" data-parsley-trigger="keyup" value="<?php echo $role_name; ?>" placeholder="" required>
							</div>
						 </div>
            ...
            <form method="post" enctype="multipart/form-data" data-parsley-validate>
                <div id="stick-here"></div>
                  <div id="stickThis" class="form-group row mg-b-0">
                     ...
                     <div class="col-9 col-sm-6 text-right">
					 <button type="submit" name="save" value="update" class="btn btn-success"><?php echo $__save_continue; ?></button>
						<input type="hidden" name="hidden_role_id" value="<?php echo $id; ?>" />
                     </div>
                  </div>
					<!-- BASIC Starting -->
                      <div id="basic">
				  <div class="divider-text"><?php echo $__hbasicinfo .' of '.getrole($id,'label'); ?></div>
					...
					$result_sub= sqlQUERY_LABEL($query_sub);
							...
							<td> <div class="custom-control custom-switch mg-t-10">
									<input type="checkbox" class="custom-control-input" name="dashboard_content[]" id="dashboard_content_<?php echo $dashboard_content_ID; ?>" <?php if($amc_email=='1' ) { echo 'checked=""' ; } ?> value="<?php echo $dashboard_content_ID;?>">
            ...
            <form method="post" enctype="multipart/form-data" data-parsley-validate>
                <div id="stick-here"></div>
                  <div id="stickThis" class="form-group row mg-b-0">
                     ...
                     <div class="col-9 col-sm-6 text-right">
					 <button type="submit" name="save" value="update_notification" class="btn btn-success"><?php echo $__save; ?></button>
						<input type="hidden" name="hidden_role_id" value="<?php echo $id; ?>" />
                     </div>
                  </div>
					<!-- BASIC Starting -->
				  ...
				  <div class="divider-text"><?php echo $__hbasicinfo .' of '.getrole($id,'label'); ?></div>
					...
					$result_sub= sqlQUERY_LABEL($query_sub);
							...
							<td> <div class="custom-control custom-switch mg-t-10">
									<input type="checkbox" class="custom-control-input" name="notification_content[]" id="notification_content_<?php echo $notification_content_ID; ?>"  value="<?php echo $notification_content_ID;?>">
