...
if($filter_customer !='' && $filter_customer !='0'){
	$filter_by_customer = " and A.user_id = '$filter_customer' ";
}

$total_bills = commonNOOFROWS_COUNT('js_customer', "");

$list_producttype_data = sqlQUERY_LABEL("SELECT A.customerID, A.customerfirst, COUNT(A.user_id), SUM(B.od_total), B.od_discount_type FROM js_customer A, js_shop_order B WHERE A.user_id = B.od_userid {$filter_by_customer} GROUP BY A.user_id") or die("#1-Unable to get PRODUCT UNIT:".sqlERROR_LABEL());

  $count_producttype_list = sqlNUMOFROW_LABEL($list_producttype_data);

  while($row = sqlFETCHARRAY_LABEL($list_producttype_data)){
	  $customersales = $row["SUM(B.od_total)"];
          ...
          <div class="col-lg-12">
				...
				<div class="card-body">
					...
					<div class="mg-md-t-20 text-left col-md-3">
						...
						<a href="<?php echo BASEPATH; ?>customersales" class="btn btn-light">Clear</a>
					</div>					
					</div>
				</div>
				</div>
		...
		<div class="col-md-4">
				...
				<div class="media mg-t-20 mg-sm-t-0 mg-md-l-1">
					...
					<div class="media-body">
					  <h6 class="tx-sans tx-uppercase tx-spacing-1 tx-color-03 tx-semibold mg-b-5 mg-md-b-8">Total Sales</h6>
					<h3 class="tx-normal tx-rubik mg-b-0 mg-r-5 lh-1"><?php echo general_currency_symbol.formatCASH($totalsales); ?></h3>
					</div> 
				</div>		
				</div>
				<div class="col-md-4">
				<div class="media mg-t-20 mg-sm-t-0 mg-sm-l-15 mg-md-l-30">
					...
					<div class="media-body">
					  <h6 class="tx-sans tx-uppercase tx-spacing-1 tx-color-03 tx-semibold mg-b-5 mg-md-b-8">Total Paid Amount</h6>
					<h3 class="tx-normal tx-rubik mg-b-0 mg-r-5 lh-1"><?php echo general_currency_symbol.formatCASH($totalsales); ?></h3>
					</div> 
				</div>	
				</div>
				...
				<div class="media mg-t-20 mg-sm-t-0 mg-sm-l-15 mg-md-l-90">
					...
					<div class="media-body">
					  <h6 class="tx-sans tx-uppercase tx-spacing-1 tx-color-03 tx-semibold mg-b-5 mg-md-b-8">Total Pending Amount</h6>
					<h3 class="tx-normal tx-rubik mg-b-0 mg-r-5 lh-1"><?php echo general_currency_symbol.formatCASH($totalsales); ?></h3>
					</div>
				</div>
				</div>
		  ...
		  <?php } if($route == 'preview'){ ?>
			...
			<input type="hidden" name="cusid" id="cusid" value="<?php echo $cusid; ?>">
				...
				<div class="mg-md-t-20 mg-r-20">
					<button type="submit" class="btn btn-info">Apply Filter</button>
					<a href="<?php echo BASEPATH; ?>customersales?route=preview&id=<?php echo $cusid; ?>" class="btn btn-light">Clear</a>
				</div>					
			</div>
			...
			<input type="hidden" name="cusid" id="cusid" value="<?php echo $cusid; ?>">
				...
				<div class="mg-md-t-20 mg-r-20">
					<button type="submit" class="btn btn-info">Apply Filter</button>
					<a href="customersales?route=preview&cusid=<?php echo $cusid; ?>" class="btn btn-light">Clear</a>
				</div>					
			</div>
