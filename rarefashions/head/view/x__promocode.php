...
if($type == 'delete') {
	...
	$get_homeiconID = sqlQUERY_LABEL("select * from js_promocode where promocode_id = '$delete_id'") or die(mysql_error());
	...
	$count_rows = sqlNUMOFROW_LABEL($get_homeiconID);
		...
		if($count_rows == '1' ) { 
			...
			echo "<hr><a href='promocode.php?action=delete&id=".$delete_id."' class='btn btn-success btn-sm'>Delete</a>";
			echo "<a href='javascript:;' class='btn btn-danger btn-sm' data-dismiss='modal' style='margin-left:20px'> Close</a></div>";
			
	} else {
		echo "<div class='col-lg-12'>Sorry cannot delete Promocode<br /><br />
			<b class='text-danger'>Reason</b>
			- No Promocode available to delete.<br /><br />
			</div>";
...
if($type == 'changestatus') {
	...
	$sqlWhere= "promocode_id=$promocode_id";

	return sqlACTIONS("UPDATE","js_promocode",$arrFields,$arrValues, $sqlWhere);
}