...
extract($_REQUEST);
	...
	$list_parentcategory_datas = sqlQUERY_LABEL("SELECT `offersimage`, `offers_name` FROM `js_offers` where offers_id ='$offers_id' and status !='3' and deleted = '0' order by offers_id DESC") or die("#1-Unable to get records:".mysql_error());
		  ...
		  while($row = sqlFETCHARRAY_LABEL($list_parentcategory_datas)){
			  ...
			  $counter++;
			  $categoryimage = $row["offersimage"];
			  $commontitle = $row["offers_name"];
		  }
...
<head>
    ...
    <meta property="og:image" content="<?php echo BASEPATH; ?>public/uploads/offers_banner/<?php echo $productmediagalleryurl; ?>" />
    <meta property="og:image:secure_url" content="<?php echo BASEPATH; ?>public/uploads/offers_banner/<?php echo $categoryimage; ?>" />
    ...
    <meta name="twitter:image" content="<?php echo BASEPATH; ?>public/uploads/offers_banner/<?php echo $categoryimage; ?>" />
	<style>
	.page-header-image {
...
<div id="load-productscreen">
	...
	?> 
		...
		<?php if(!empty($categoryimage)){
			?>
			<div class="page-header-image text-center" style="background-image: linear-gradient(to bottom, rgba(0,0,0,0) 70%, rgba(0, 0, 0, 0.64) 100%), url('<?php echo BASEPATH; ?>public/uploads/offers_banner/<?php echo $categoryimage;?>')">
...
<script>
	...
	$('.custom-control-input').click(function(){
       getresult("ajax/offer_pagination.php?s=<?php echo $search_value; ?>&token=<?php echo $offers_id; ?>");
    });
