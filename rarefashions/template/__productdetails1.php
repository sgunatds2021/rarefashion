<main class="main">
		...
		<div class="container d-flex align-items-center">
			<ol class="breadcrumb">
				...
				foreach($cats as $cat) {
					...
					$__category_parent = getSINGLEDBVALUE('categoryparentID', " categoryID='$cat' and deleted='0' and status = '1'", 'js_category', 'label');
					if($__category_parent == '0'){
						$categories_parent .= " <a href='shop.php?token=".$cat."-".convertSEOURL($__category_title)."'>" . $__category_title . "</a>,";
					} else {
						$categories .= " <a href='shop.php?token=".$cat."-".convertSEOURL($__category_title)."'>" . $__category_title . "</a>,";
					}
				}
				$categories_parent = substr($categories_parent, 0, -1);
				$categories = substr($categories, 0, -1);
				if($categories_parent != '') {
				?>
				<li class="breadcrumb-item"><?php echo $categories_parent; ?></li>
				<?php } 
				if($categories != '') {
				?>
				<li class="breadcrumb-item"><?php echo $categories; ?></li>
				<?php } ?>
						...
						<div class="product-details " >
							...
							<div class="product-price" id="product-price" style="display:none">
								 <p class="product-price">
									<span id="variant_price" class="kreen-Price-amount amount"><span class="kreen-Price-currencySymbol"></span></span>   &nbsp;
								<del><span id="variant_price1" class="kreen-Price-amount amount text-light"><span class="kreen-Price-currencySymbol text-light"></span></span></del></p>							
		                   </div>
                          ...
                          <div class="clearfix"></div>
							...
							?>
								...
								<div class="product-size" id="product-size">
									...
									<i class="icon-check d-none" id="icon<?php echo $variant_ID; ?>" style="  position: relative !important;right: -30px !important; color:#403a3a  !important;  "></i>
									<a id="variant_size_display<?php echo $variant_ID; ?>" class="variant_size_class" href="javascript:;"  Onclick="getSIZE('<?php echo $variant_ID; ?>','<?php echo $variant_msp_price+$tot_tax_value; ?>','<?php echo $variant_mrp_price; ?>','<?php echo $variant_size_count; ?>');" title="<?php echo general_currency_symbol; ?><?php echo number_format($variant_msp_price+$tot_tax_value,2);?>"  >  <span><?php echo $variant_name; ?><span></a>
									<?php
									}
									?>
							...
							// $check_prdt_variant_datas_selected = sqlQUERY_LABEL("SELECT * FROM `js_productvariants` where deleted = '0' and status = '1' and variantstockstatus='1' and `parentproduct`='$productID' LIMIT 1") or die("Unable to get records:".mysqli_error());	
									...
									// }
										?>
										<?php 
								$sql_query_token = sqlQUERY_LABEL("SELECT `prdt_id`,`offers_id` FROM `js_offers_items` where `prdt_id`='$productID' and deleted = '0' and status = '2'") or die("#STATUS-SELECT: Getting Status: ".sqlERROR_LABEL());
											while($set_sql_query_token = sqlFETCHARRAY_LABEL($sql_query_token)){
												$productID_token = $set_sql_query_token['prdt_id'];
												$offer_id[] = $set_sql_query_token['offers_id'];
											}
											
											$offers_id = implode("','", $offer_id);
											$PRODUCTS_offers_id = implode(",", $offer_id);
										 
									//print_r($offers_id)	;
							$offers_type[] =  getSINGLEDBVALUE('offers_type', " offers_id IN('$offers_id') and deleted = '0' and status = '2'", 'js_offers', 'label');
							$offers_type = implode("','", $offers_type);
							$PRODUCTS_offers_type = implode(",", $offers_type);
						
							   
						?>	
							<input type="hidden" name="offer_id" id="offer_id" value="<?php echo $PRODUCTS_offers_id; ?>"/>
							<input type="hidden" name="offer_type" id="offer_type" value="<?php echo $PRODUCTS_offers_type; ?>"/>
							...
							<span class="text-danger" id="size_alert" style="display:none"> Select Size before Add </span>
							<div class="product-details-action" id="product-details-action">
							 
							<button type="button" id="add_to_cart_in_details" name="add_to_cart_in_details" value="add_to_cart_in_details" class="btn-product btn-cart cart-btn" <?php echo $cartbutton_disable; ?>>ADD TO CART</button>
								...
								<div class="details-action-wrapper">
									<a onClick="product_wishlist(<?php echo $productID; ?>, <?php echo $variant_ID_selected; ?>)" href="javascript:void(0);" class="btn-product btn-wishlist" title="Add to Wishlist" ><span>ADD TO WISHLIST</span></a>
								</div><!-- End .details-action-wrapper -->
							</div><!-- End .product-details-action -->
							<?php if($offer_id !=''){	
							?>
							<div class="header_details" style="">
								<h5>Available Offers</h5>  
							<?php
								foreach ($offer_id as $key) {
							...
							<div class="header_details" style="">
								...
								<li> <?php echo $offer_name ?>  . <u><a href="offers.php?type=<?php echo $offer_type ?>&offerID=<?php echo $key ?>">View Offer Available Product</a></u></li>
								<?php /*?><li><b>Offer Details : </b> </li>
								<li><?php echo $offer_description ?> </li><?php */?>
								</ul> 
							</div>
							<?php 
								} 
							...
							<div class="product-details-footer">
								<div class="product-cat">
									...
									foreach($cats as $cat) {
										$cat = trim($cat);
										$__category_title = getSINGLEDBVALUE('categoryname', " categoryID='$cat' and deleted='0' and status = '1'", 'js_category', 'label');
										$categories .= " <a target='_blank' href='shop.php?token=".$cat."-".convertSEOURL($__category_title)."'>" . $__category_title . "</a>,";
									}
					  ...
					  $count_product_list = sqlNUMOFROW_LABEL($list_product_datas);
						...
						<div class="product product-7 text-center">
							...
							<div class="product-body">
								...
								<?php echo checkproductPRICE($productID_);?>
    ...
    <div class="container fix">
        <div class="row">
            <?php
		  //  print_r($upsellproduct_list);die;
			...
			$selectedproduct_id_ = getSINGLEDBVALUE('productID', "productsku = '$selectedproduct_id_' and deleted = '0' and status = '1'", 'js_product', 'label');
...
// echo $counter;
            ...
            <div class="product product-7 text-center">
                ...
                <div class="product-body">
                    ...
                    <div class="product-price"><?php echo checkproductPRICE($product_ID);?></div>
                </div>
            </div>
            </div>
            <?php } 
