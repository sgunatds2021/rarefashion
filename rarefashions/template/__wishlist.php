            ...
            <div class="page-content">
            	<div class="container">
					<table class="table table-wishlist table-mobile">
						...
						<tbody id="wishitem">
								<?php 
								//echo "SELECT * FROM js_wishlist WHERE `wish_userid` = '$logged_user_id' AND `deleted` = '0'";
									...
									while($row = sqlFETCHARRAY_LABEL($result))
										...
										//$count_product_variant_id = sqlNUMOFROW_LABEL($product_variant_id);
										
										if($product_variant_id > 0) {
											
											$list_product_datas = sqlQUERY_LABEL("SELECT js_product.productID, js_product.productsku,js_product.producttitle,variant_msp_price as sellingprice,js_productvariants.variant_opening_stock as opening_stock ,js_productvariants.variant_mrp_price as mrp_price ,js_productvariants.variant_available_stock as available_stock ,js_productvariants.variantstockstatus as stockstatus,js_productvariants.variant_taxsplit1 as taxsplit1,js_productvariants.variant_taxsplit2 as taxsplit2 FROM js_product,js_productvariants WHERE js_product.`productID` = '$wish_prdtid' and js_productvariants.`variant_ID` = '$product_variant_id'") or die("#1-Unable to get records:".sqlERROR_LABEL());
										}else{
										
											$list_product_datas = sqlQUERY_LABEL("SELECT `productID`, `productsku`, `producttitle`, `productsellingprice` as sellingprice, `productMRPprice` as mrp_price, `productstockstatus` as stockstatus, `productavailablestock` as available_stock FROM `js_product` where productID = '$wish_prdtid' and deleted = '0' and status = '1'") or die("#1-Unable to get records:".sqlERROR_LABEL());
										}
											...
											while($row = sqlFETCHARRAY_LABEL($list_product_datas)){
											  ...
											  if($product_variant_id > 0){
												  ...
												  $variant_taxsplit1    = $row["taxsplit1"];
										          $variant_taxsplit2    = $row["taxsplit2"];
												  $variant              = $variant_taxsplit1 +$variant_taxsplit2;
												  $sellingprice         = $row["sellingprice"];
												  $productsellingprice  = formatCASH($sellingprice +$variant);
											  }else{
												  $productsellingprice = formatCASH($row["sellingprice"]);
											  }
											//  $productsellingprice = formatCASH($row["sellingprice"]);
											  $productMRPprice = formatCASH($row["mrp_price"]);
											  $productavailablestock = $row["available_stock"];
											  $productstockstatus = $row["stockstatus"];  //stock status
